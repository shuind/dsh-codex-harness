...
describe('preset product workflows', () => {
  ...
  it('saves DSH template edits in an ordinary copy without enabling Codex or changing its source', async () => {
    ...
    expect(copy.codex).toBe(false)
    expect(copy.promptSections.find(s => s.id === 'harness:source')).toMatchObject({ text: 'Read {{sourceRoot}}' })
    expect(copy.promptSections.some(s => s.id === 'app:web-surface')).toBe(false)
  ...
  it('does not add DSH templates to a complete Persona and preserves that when copied', async () => {
    ...
    const enhancedComposition = enhanceComposition(minimal)
    const enhanced = readPromptSections(enhancedComposition)
    expect(enhanced.find(s => s.id === 'harness:source')).toBeUndefined()
    expect(enhanced.find(s => s.id === 'app:web-surface')).toBeUndefined()
    expect(parse(enhancedComposition).at(-1)?.config.presetSections).toEqual({ harnessSourcePrompt: '', webSurfacePrompt: '' })
  })
